package com.jin.springbootdemo.cofig;

import com.jin.springbootdemo.bean.ShiroUser;
import com.jin.springbootdemo.cache.DBCache;
...
public class AuthRealm  extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token)
        ...
        ShiroUser user = Optional.ofNullable(DBCache.USERS_CACHE.get(principal)).orElseThrow(UnknownAccountException::new);
        if (!user.isLocked()) {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principal) {
        Session session = SecurityUtils.getSubject().getSession();
        ShiroUser user = (ShiroUser) session.getAttribute("USER_SESSION");
